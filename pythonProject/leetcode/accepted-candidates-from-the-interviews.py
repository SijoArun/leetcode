...
def accepted_candidates(candidates: pd.DataFrame, rounds: pd.DataFrame) -> pd.DataFrame:
    score=rounds.groupby(['interview_id'])['score'].sum()
    merged=candidates.merge(score,on='interview_id',how='left')
    merged=merged[(merged['years_of_exp'] >= 2)  & (merged['score'] >15)].reset_index()
    return merged.filter(items={'candidate_id'})
